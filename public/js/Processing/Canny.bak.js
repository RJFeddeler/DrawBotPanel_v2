(function() {
  ...
  CannyJS.invert = function(imgData) {
    var copy;
  ...
  CannyJS.sobel = function(imgData, kernel) {
    ...
    /**
   * appy roberts filter to image data
   * @param {object} GrayImageData object
   * @return {object} GrayImageData object
   */

  CannyJS.roberts = function(imgData, kernel) {
    var copy, xFiler, yFiler;
    yFiler = [[1, 0], [0, -1]]; /// WILL THIS WORK WITH ONLY 2???? /////////////////////////////
    xFiler = [[0, 1], [-1, 0]];
    copy = imgData.copy();
    copy.fill(0);
  ...
  CannyJS.prewitt = function(imgData, kernel) {
    var copy, xFiler, yFiler;
    yFiler = [[-1, 0, 1], [-1, 0, 1], [-1, 0, 1]];
    xFiler = [[-1, -1, -1], [0, 0, 0], [1, 1, 1]];
    copy = imgData.copy();
    copy.fill(0);
    imgData.eachPixel(3, function(x, y, current, neighbors) {
      var ghs, gvs, i, j, _i, _j;
  ...
  CannyJS.canny = function(canvas, ht, lt, sigmma, kernelSize) {
    ...
    hyst = CannyJS.hysteresis(nms, ht, lt);
